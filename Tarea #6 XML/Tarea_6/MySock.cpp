 ...
 *   02.10.2008 Gaspar Fernández   Initial release
 *   05.11.2008 Gaspar Fernández   Fixed compilation on some systems
 *   07.27.2010 Gaspar Fernández   Some minor bugs corrected
 ********************************************************************************/ 
#include "MySock.h"
#include <stdio.h>
#include <string.h>
...
MySock::MySock(string uri)
{
  ...
  if ((pos!=string::npos) && (pos==uri.find("/", 0)) && (pos>0))
    {
      ...
      if (protostr=="http")
	{
	  ...
	  if (this->connected)
	    this->SendData("GET "+params+" HTTP/1.0\n\n");
	}
      else
	{
...
void MySock::MakeConnection(string server, int port)
{
  ...
  this->connected=(this->error==NO_ERROR);
}
...
HTTP_Request *MySock::GetHTTPData()
{
  ...
  if (this->error==NO_ERROR)
	...
	http->data=txtData.substr(pos2+2); // Skip las \r\n
      return http;
    }
  return NULL;
}
